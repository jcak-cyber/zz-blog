  ...
  return (
    <div className="pb-28">
      <section className="hero-band relative mt-4 overflow-hidden px-6 py-10 md:mt-8 md:px-10 md:py-14">
        <div className="relative z-raised flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          <div className="flex items-start gap-5">
            ...
            <div>
              <h1 className="font-brush text-balance text-4xl md:text-5xl">{author.nickname}</h1>
              <p className="mt-2 text-sm text-[#e7e0d4]/70">@{author.username}</p>
              {author.bio ? (
                <p className="mt-4 max-w-xl whitespace-pre-wrap text-pretty text-sm leading-7 text-[#e7e0d4]">
                  {author.bio}
                </p>
              ) : (
      ...
      <section className="pt-10 md:pt-14">
        <div className="mb-6 flex items-baseline justify-between gap-4 px-1">
          <h2 className="font-display text-balance text-xl md:text-2xl">公开手稿</h2>
          <p className="text-sm tabular-nums text-[var(--ink-faint)]">{author.posts.length} 篇</p>
        </div>
        <PostList
          posts={author.posts}
          layout="list"
          hideAuthor
          emptyAction={{ href: '/', label: '去看文章列表' }}
        />
      </section>
    </div>
  );
